...
width, height = A4
output_path="goodenough_full_report.pdf"
...
def generate_full_pdf(manual_info=None, output_path=None, input_pdf_paths=None, INT_data=None, TCI_scores=None, PAT_scores=None, TCI_filename=None, PAT_filename=None):
    ...
    PAT_fileflag = 1 if PAT_filename is not None else 0
    TCI_fileflag = 1 if TCI_filename is not None else 0

    if INT_fileflag == 1:
        ## PAGE 04 지능검사 요약 호출
        fsiq = INT_data["fsiq"]
        percentile = INT_data["percentile"]
        strength_label = INT_data["strength_label"]
        strength_score = INT_data["strength_score"]
        weakness_label = INT_data["weakness_label"]
        weakness_score = INT_data["weakness_score"]
        sub_scores = INT_data["sub_scores"]
        ci_min = INT_data["ci_min"]
        ci_max = INT_data["ci_max"]
        diagnosis_labels = INT_data["diagnosis_labels"]
        subtest_scores = INT_data["subtest_scores"]
        index_scores_all = INT_data.get("index_scores_all", {})
        IntelligenceDomain = INT_data["IntelligenceDomain"]
    ...
    def makePageNum(pageNum):
        ...
        # 페이지 번호 두 자리 변환 (1 → 01, 12 → 12)
        page_num_str = str(pageNum).zfill(2)

        c.setFont("Pretendard-Regular", 9)
        c.setFillColor(HexColor("#A9A9A9"))
        c.drawRightString(width - 47.5, 40, page_num_str)
    def make_back_logo():
        try:
    ...
    def makePage01():
        ...
        if INT_fileflag == 1:
            items.append((f"{count:02}", "웩슬러 지능검사", "Korean-Wechsler Intelligence Scale"))
            count += 1

        if TCI_fileflag == 1:
            items.append((f"{count:02}", "기질 및 성격검사", "Temperament and Character Inventory"))
            count += 1

        if PAT_fileflag == 1:
            items.append((f"{count:02}", "부모 양육태도 검사", "Parenting Attitude Test – Second Edition"))
            count += 1
    ...
    def makePage05():
        ...
        def draw_subtest_scores_final(c, domains, subtests, x, y):
            if ("4세 이상" in INT_filename or "4세이상" in INT_filename) and ("wppsi" in INT_filename.lower()):
                print("wppsi 4세 이상")
                # --- 상수 ---
                bar_width = 9
                bar_gap = 27
                ...
                domain_gap = 19
            elif ("4세 미만" in INT_filename or "4세미만" in INT_filename) and "wppsi" in INT_filename.lower():
                print("wppsi 4세 미만")
                # --- 상수 ---
                bar_width = 20
                bar_gap = 45
                ...
                full_chart_height = chart_height + domain_label_margin
                domain_gap = 41
            elif "wais" in INT_filename.lower():
                print("wais")
                bar_width = 10
                bar_gap = 30
                bar_unit = 9
            ...
            half_gap = (bar_gap - bar_width) / 2
            label_x = x - 40  # 왼쪽 수준 라벨 시작
            x_content = x + half_gap  # 첫 막대 왼쪽 여백 = half_gap
            ...
            # 전체 통계
            num_domains = len([idxs for idxs in domains.values() if idxs])
            total_bars = sum(len(idxs) for idxs in domains.values())

            # --- 도메인/막대 ---
            x_cursor = x_content
            # first_domain = True
            #
            # c.line(x, y, x, y + full_chart_height - 5)

            for domain, idxs in domains.items():
                if not idxs:
                    continue

                # 도메인 라벨 중앙(첫/끝 막대 '중심'의 평균)
                first_center = x_cursor + 0.5 * bar_gap
                last_center = x_cursor + (len(idxs) - 0.5) * bar_gap
                center_x = (first_center + last_center) / 2

                c.setFont("Pretendard-Bold", 8)
                c.setFillColor(HexColor("#000000"))
                c.drawCentredString(center_x, y + chart_height + domain_label_margin - 10, domain)

                # 그룹 경계 점선
                split_x = x_cursor - (domain_gap / 2)
                ...
                for j, gi in enumerate(idxs):
                    sub_label, score = subtests[gi]  # ⚠ 이름 충돌 방지: sub_label
                    bx = x_cursor + j * bar_gap + domain_gap / 3
                    bh = int(score) * bar_unit
                    cx = bx + bar_width / 2
                    ...
                    c.setFillColor(HexColor("#000000"))
                    c.setFont("Pretendard-Regular", 6)
                    c.drawCentredString(cx, y - 10, sub_label)

                x_cursor += len(idxs) * bar_gap + domain_gap

            # ✅ 오른쪽 끝 x: 도메인 간격 포함 + 마지막 half_gap
            end_x = x_content + total_bars * bar_gap + max(0, num_domains - 1) * domain_gap + half_gap

            # --- 수평 기준선 & 라벨 ---
            for start, end, lbl in level_ranges:
                top_y = y + end * bar_unit
                center_y = y + (start - 1) * bar_unit + (end - start + 1) * bar_unit / 2
                ...
                c.setFont("Pretendard-Bold", 8);
                c.setFillColor(HexColor("#000000"))
                c.drawCentredString(label_x + 21, center_y - 4, lbl)

            # --- 테두리 ---
            c.setDash()
            c.setStrokeColor(HexColor("#888888"));
            c.setLineWidth(0.8)
            c.line(label_x, y, label_x, y + full_chart_height - 5)  # left
            c.line(end_x, y, end_x, y + full_chart_height - 5)  # right
            c.line(label_x, y, end_x, y)  # bottom


        # 하위검사 점수
        def draw_subtest_scores_final_WISC(c, domains, subtests, x, y):
        ...
        if "wais" in INT_filename.lower():
            domain_order = ["언어이해", "지각추론", "작업기억", "처리속도"]
        # 변환 결과
        subtests = []
        domains = {}
        index = 0
        ...
        if ("wais" in INT_filename.lower())  or ("wppsi" in INT_filename.lower()):
            for domain in domain_order:
                domain_subtests = []
                for key, value in subtest_scores.items():
                    if key.startswith(domain + "_"):
    ...
    def makePage06():
        ...
        def draw_gauge_section(c, x, y, title, score, level, description):
            ...
            c.setFillColor(HexColor("#000000"))
            text_y = y + 60
            draw_multiline_text(c,x +165, text_y, description, max_width=300, font_size=8, font_name="Pretendard-SemiBold")

        start_y = height - 250

        domains_list = [key.split('_')[0] for key in subtest_scores.keys()]
        ...
        for i, (title, score, level, desc, count) in enumerate(labels):
            draw_gauge_section(c, x=65, y=start_y - gap, title=title, score=score, level=level, description=desc)
            gap += 135
            if count > 2:
                gap += 10 * (count - 2)
                print(gap)
